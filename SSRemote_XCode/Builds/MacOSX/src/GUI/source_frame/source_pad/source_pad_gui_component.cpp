...
, grid(nullptr)
, head(new SSR::Head_gui_component())
...
, current_discrete_y_position(0.0f)
{
  ...
  head->setBounds(width/2.0f - source->getWidth()/2.0f, heigth/2.0f - source->getHeight()/2.0f, head->getWidth(), head->getHeight());
    
//    head->setTransform(AffineTransform::rotation
...
void SSR::Source_pad_gui_component::move_head(const float degrees){
    
    
    head->setTransform(AffineTransform::rotation
                       (-degreesToRadians(degrees - 90.0f),
