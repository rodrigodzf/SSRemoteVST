...
void SSR::Head_frame_GUI_component::buttonClicked(juce::Button* buttonThatWasClicked)
{
    ...
    if (buttonThatWasClicked == save_button.get()) {
        if (!processor->is_writing_xml()) {
            processor->save_xml(true);
            save_button->setToggleState(true, juce::dontSendNotification);
        } else {
            processor->save_xml(false);
            save_button->setToggleState(false, juce::dontSendNotification);
        }
    }
